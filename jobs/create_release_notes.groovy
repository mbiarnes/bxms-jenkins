...
#copy the template 
cp ./brms-release/brms-release-notes-template.html ./${product_version}-release-notes.html
...
echo "cve serach link: Please click the following link"
...
while read -r line; do
   ...
   sed -i "/<!--table_1_appending_mark-->/ a $table_1" ${product_version}-release-notes.html
...
while read -r line; do
   ...
   sed -i "/<!--table_2_appending_mark-->/ a $table_2" ${product_version}-release-notes.html
done < ./jql_search_data.txt

sed -i "s/\\${kie_version}/$kie_version/g" ${product_version}-release-notes.html
sed -i "s/\\${product_artifact_version}/$product_artifact_version/g" ${product_version}-release-notes.html
sed -i "s/\\${product_version}/$product_version/g" ${product_version}-release-notes.html
'''
