...
product_assembly_maven_repo_url=http://blabla
...
adoc_file=open('\${qe_smoketest_report_path}.adoc', 'w')
adoc_file.write('+\\n')
...
def jobDefinition = job("${PRODUCT_NAME}-monitoring-cimessage") {
    ...
    triggers{
        ciBuildTrigger {
            selector("label='bxms-ci' OR (CI_TYPE='brew-tag' AND ( CI_NAME='org.jboss.ip-bxms-maven-repo-root' OR label='bxms-ci' OR CI_NAME='org.jboss.ip-brms-bpmsuite-assembly' OR CI_NAME='org.jboss.brms-bpmsuite.patching-patching-tools-parent'))")
