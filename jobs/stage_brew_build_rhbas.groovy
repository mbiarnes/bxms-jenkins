...
ip-tooling/maven-to-stage.py --version=\${product2_artifact_version} --override-version \${product2_shipped_file_deliver_version} --maven-repo \${product2_assembly_maven_repo_url} \
...
def jobDefinition = job("${RELEASE_CODE}-stage-brew-build-rhbas") {
            ...
            server('publish server') {
                    ...
                    transferSet {
                        ...
                        execCommand('if [ "${CLEAN_STAGING_ARTIFACTS}" = "true" ];then \n' +
                                        'rm -vrf ~/staging/${product2_staging_path}/* \n' +
                                    'fi')
                    }
                    // Adds a target server.
                    verbose(true)
